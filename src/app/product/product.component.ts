...
@Component({
  ...
  template: `
    <ng-container *ngIf="product$ | async as product; else loading">
      <ng-container *ngIf="product.uid !== 'error'; else error">
        <p>uid: {{  product.uid  }}</p>
...
export class ProductComponent {
  ...
  product$: Observable<Product | undefined> = this.activatedRoute.params.pipe(
    switchMap(params =>
      this.store.collection('products').doc<Product>(params['uid']).snapshotChanges().pipe(
        map(snapshot =>
          snapshot.payload.exists ? snapshot.payload.data() : {uid: 'error'}
        )
      )
    )
  );
